...
def plot_multik_cdfs(
    output_npz: str,
    snapshot_npz: str = None,
    ...
    # ---- Panel 2: tail context ----
    if snapshot_npz is not None:
        _, x_tail = load_snapshot_tail(snapshot_npz, k_max)
        if x_tail is not None:
            rel = np.arange(-len(x_tail) + 1, 1)  # ends at 0
            ax2.plot(rel, x_tail, marker="o")
            ax2.axvline(0, ls="--")
            ax2.set_title(f"Last {len(x_tail)} observed values (conditioning context up to k={k_max})")
